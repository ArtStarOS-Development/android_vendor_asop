...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/ArtStarOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        adb wait-for-online
        echo "Found device"
        if (adb shell getprop ro.artstar.device | grep -q "$ARTSTAR_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $ARTSTAR_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/ArtStarOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-online
    if (adb shell getprop ro.artstar.device | grep -q "$ARTSTAR_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.artstar.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/asop/build/tools/repopick.py $@
}
