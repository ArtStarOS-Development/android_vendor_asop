...
function setup_vendor() {
    ...
    export ASOP_ROOT="$3"
    if [ ! -d "$ASOP_ROOT" ]; then
        echo "\$ASOP_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$ASOP_ROOT/$OUTDIR" ]; then
        mkdir -p "$ASOP_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$ASOP_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$ASOP_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$ASOP_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$ASOP_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ASOP_ROOT"/prebuilts/tools-custom/common/smali/baksmali.jar
        export SMALIJAR="$ASOP_ROOT"/prebuilts/tools-custom/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$ASOP_ROOT"/prebuilts/tools-custom/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$ASOP_ROOT"/prebuilts/tools-custom/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$ASOP_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$ASOP_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$ASOP_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$ASOP_ROOT"/vendor/asop/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$ASOP_ROOT"/"$OUTDIR"/radio
